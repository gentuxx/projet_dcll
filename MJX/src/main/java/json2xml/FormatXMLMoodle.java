...
<<<<<<< HEAD
	...
	/**
	 * Permet de convertir les balises de contenu en attribut et retourne le code XML correspondant
	 * <balise><type> Banzai </type></balise> devient <balise type="Banzai></balise>"
	 ...
	 * @throws JSONException
	 */	
	private static String baliseToAttribute(JSONObject contenu, String balise, String... balisesAConvertir) throws JSONException{
		Object contenuTestType = contenu.get(balise);
		
		if(contenuTestType instanceof JSONArray){
			return baliseToAttribute(contenu.getJSONArray(balise),balise,balisesAConvertir);
		}
		...
		for(int i = 0; i < balisesAConvertir.length; i++){
			if(contenuTmp.has(balisesAConvertir[i])){
				returnString += " " + balisesAConvertir[i] + "=\"" + contenuTmp.get(balisesAConvertir[i]) +"\"";
	...
	private static String baliseToAttribute(JSONArray contenu, String balise, String... balisesAConvertir) throws JSONException{
		String returnString = "";
